...
void verifyInput(){
	...
	while (plainText[i] != '\0'){
		...
		if (x){
			fprintf(stderr, "Bad characters found in plain text input\n");
			exit(1);
		}
		i++;
	}
...
int main(int argc, char *argv[]){
	...
	int term = strstr(decrypted, "**") - decrypted;
	decrypted[term] = '\0';


	//Report error if connected to wrong server
	if (strcmp(decrypted, "Connected to wrong server") == 0){
